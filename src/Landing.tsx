...
const BAIT_MARKETS = [
  {
    ...
    sentimentLabel: 'Yes',
    poolSize: '1,200,000',
    image: 'https://images.unsplash.com/photo-1532986427357-19fbcc0fa5ea?q=80&w=800&auto=format&fit=crop', // F1 Driver/Car vibe
  },
  {
    id: 'crypto-mock',
    ...
    sentimentLabel: 'Yes',
    poolSize: '2,500,000',
    image: 'https://images.unsplash.com/photo-1621416894569-0f39ed31d247?q=80&w=800&auto=format&fit=crop', // 3D Gold Bitcoin
  },
  {
    id: 'fashion-mock',
    ...
    sentimentLabel: 'Hot',
    poolSize: '800,000',
    image: 'https://images.unsplash.com/photo-1492288991661-058aa541ff43?q=80&w=800&auto=format&fit=crop', // High Fashion Runway
  }
]
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-[#C5A880]/20 font-sans relative">
      ...
      {/* BACKGROUND GLOWS */}
      <div className="absolute top-0 left-0 w-[600px] h-[600px] bg-[#C5A880]/5 rounded-full blur-[150px] pointer-events-none"></div>

      {/* 1. TOP NAVIGATION */}
      <header className="max-w-[1400px] mx-auto px-6 py-6 flex justify-between items-center relative z-20">
        ...
        <button 
          ...
          className="text-xs font-bold text-gray-300 bg-transparent px-5 py-2.5 rounded-lg border border-[#ffffff20] hover:bg-[#ffffff0a] hover:text-white transition-all tracking-wider"
        >
          Enter Warzone
        </button>
      </header>

      {/* 2. THE TICKER (Directly below Nav per design) */}
      <div className="w-full bg-[#111111] py-3 overflow-hidden whitespace-nowrap relative z-20 border-y border-[#ffffff0a]">
        <div className="animate-marquee text-[11px] tracking-widest text-[#C5A880] font-bold uppercase flex items-center gap-4">
          <span className="text-white">SATOSHIN</span> staked 5,000 KSh on BTC {'>'} $100k • <span className="text-white">USER123</span> bet 1,000 USDC on Arsenal Win • <span className="text-white">TRADER99</span> placed 2,500 on F1 Hamilton Podium • 
          <span className="text-white ml-4">SATOSHIN</span> staked 5,000 KSh on BTC {'>'} $100k • <span className="text-white">USER123</span> bet 1,000 USDC on Arsenal Win • <span className="text-white">TRADER99</span> placed 2,500 on F1 Hamilton Podium •
        </div>
      </div>

      {/* 3. ASYMMETRICAL HERO SECTION */}
      <main className="max-w-[1400px] mx-auto px-6 py-16 lg:py-24 grid grid-cols-1 xl:grid-cols-2 gap-16 items-center relative z-10">
        
        {/* LEFT SIDE: THE TEXT & CTA */}
        <div className="flex flex-col items-start text-left">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#141414] border border-[#ffffff10] text-gray-400 text-[10px] font-bold tracking-widest uppercase mb-8 shadow-md">
            <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span>
            LIVE WARZONES ACTIVE
          </div>
          
          <h2 className="text-6xl lg:text-[80px] font-bold tracking-tighter mb-6 leading-[0.95] uppercase">
            Put your <br />
            money <br />
          ...
          <button 
            onClick={() => setShowAuth(true)}
            className="bg-[#C5A880] text-[#0a0a0a] font-bold text-sm py-4 px-8 rounded-xl transition-all hover:bg-[#b0956e] flex items-center gap-3 group shadow-[0_0_30px_rgba(197,168,128,0.2)]"
          >
            Start a Warzone <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </button>
        </div>

        {/* RIGHT SIDE: THE EXACT TERMINAL MOCKUP */}
      ...
      {/* 4. THE MARKET BAIT CARDS */}
      <section className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-8 pb-24">
        
        {/* Category Pills */}
        <div className="flex gap-3 overflow-x-auto pb-6 no-scrollbar mb-6 select-none">
           {['All', 'Sports', 'Crypto', 'F1', 'Fashion', 'Politics'].map(cat => (
             <button key={cat} className={`px-6 py-2.5 rounded-xl text-sm font-semibold transition border whitespace-nowrap ${cat === 'All' ? 'bg-[#C5A880] text-[#0a0a0a] border-[#C5A880]' : 'bg-transparent text-gray-400 border-[#ffffff20] hover:border-[#ffffff40] hover:text-white'}`}>
               {cat}
             </button>
           ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {BAIT_MARKETS.map(market => (
            <div key={market.id} onClick={() => setShowAuth(true)} className="bg-transparent rounded-3xl border border-[#ffffff15] overflow-hidden group cursor-pointer transition-all duration-300 hover:border-[#C5A880]/50 hover:-translate-y-1 shadow-lg flex flex-col p-4 pb-6">
              ...
              {/* Image */}
              <div 
                className="h-64 rounded-2xl bg-cover bg-center mb-5"
                style={{ backgroundImage: `url(${market.image})` }}
              ></div>
              
              {/* Card Body */}
              <div className="flex-grow flex flex-col px-2">
                <h3 className="text-2xl font-bold text-white mb-6 leading-tight group-hover:text-[#C5A880] transition-colors">{market.title}</h3>
                ...
                <div className="mt-auto">
                   ...
                   <div className="space-y-4">
                     ...
                     <button className="w-full bg-[#C5A880] text-[#0a0a0a] text-sm font-bold py-3.5 rounded-xl hover:bg-[#b0956e] transition">
                       Trade Now
                     </button>
                   </div>
                </div>
      ...
      <footer className="w-full bg-[#0a0a0a] border-t border-[#ffffff10] py-12 relative z-10">
         <div className="max-w-[1400px] mx-auto px-8 grid grid-cols-2 lg:grid-cols-4 gap-8 text-center">
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-widest mb-2 font-semibold">Total Locked Value:</p>
              <p className="text-3xl font-bold text-white">$12,500,000</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-widest mb-2 font-semibold">Active Traders:</p>
              <p className="text-3xl font-bold text-white">25,456</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-widest mb-2 font-semibold">Avg Return:</p>
              <p className="text-3xl font-bold text-[#C5A880]">18%</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-widest mb-2 font-semibold">Protocol Fee:</p>
              <p className="text-3xl font-bold text-white">3%</p>
            </div>
         </div>
      </footer>

      {/* --- AUTH MODAL OVERLAY (Matches Design 1:1) --- */}
      {showAuth && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in-up">
          <div className="bg-[#111111] border border-[#C5A880]/30 rounded-3xl p-8 sm:p-10 w-full max-w-sm text-center shadow-[0_0_50px_rgba(197,168,128,0.15)] relative">
            ...
            <div className="relative z-10">
              <h2 className="text-3xl font-bold text-white mb-8">Enter the Arena</h2>
              
              <div className="space-y-4 mb-6">
                <button className="w-full bg-[#D5C09F] text-[#0a0a0a] font-bold py-3.5 rounded-xl hover:bg-[#C5A880] transition text-sm">
              ...
              <p className="text-xs text-gray-500">
                New here? <button className="underline text-gray-400 hover:text-white transition">Create an account</button>
              </p>
            </div>

            {/* Hidden close button to click out of it (for dev UX) */}
            <button 
              onClick={() => setShowAuth(false)} 
              className="absolute top-4 right-5 text-gray-500 hover:text-white text-xl"
            >
              ×
            </button>

            {/* We render your actual <Auth /> hidden or integrate it later. For 1:1 visual match, we show the dummy buttons above. 
                If you want your actual Supabase auth to show here instead, replace the buttons above with <Auth /> */}
