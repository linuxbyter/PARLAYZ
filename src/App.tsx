...
export default function App() {
  ...
  const initiateMatch = (offer: Bet) => {
    ...
    if ((profile?.wallet_balance || 0) < liability) return showToast(`Insufficient balance. Need ${liability.toLocaleString()} KSh.`)
        ...
        {showAI && (
            <div className="w-[320px] h-[480px] bg-[#111111] border border-[#C5A880]/30 rounded-[2rem] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 zoom-in-95 duration-300">
                ...
                <div className="p-4 bg-[#111111] border-t border-[#ffffff05] flex gap-2">
                    <input value={aiInput} onChange={e => setAiInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAISend()} placeholder="Ask me anything..." className="flex-1 bg-[#0a0a0a] border border-[#ffffff10] rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-[#C5A880] transition" />
      ...
      {showProfileSetup && (
        <div className="fixed inset-0 z-[150] bg-[#0a0a0a]/98 flex items-center justify-center p-4 backdrop-blur-xl">
            <div className="bg-[#111111] border border-white/5 p-10 rounded-[3rem] max-w-sm w-full text-center shadow-2xl relative overflow-hidden">
                ...
                <h2 className="text-3xl font-black mb-2 text-white tracking-tighter italic">PARLAYZ</h2>
                <p className="text-gray-500 text-[10px] mb-10 uppercase tracking-[0.3em] font-bold">Pick an Icon to Enter the Arena</p>
      ...
      <main className="max-w-6xl mx-auto px-4 py-10">
        ...
        <div className="flex gap-2 mb-12 overflow-x-auto pb-4 no-scrollbar">
            {['orderbook', 'markets', 'wagers', 'leaderboard'].map(v => (
                <button key={v} onClick={() => setActiveView(v as any)} className={`px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.25em] transition-all duration-500 ${activeView === v ? 'bg-white text-black shadow-xl' : 'bg-[#111111] text-gray-600 border border-[#ffffff05] hover:border-gray-500'}`}>
                        ...
                        return (
                            <div key={offer.id} className="bg-[#111111] border border-[#ffffff0a] rounded-[2.5rem] p-8 relative overflow-hidden group hover:border-[#C5A880]/40 transition-all duration-500 shadow-2xl">
                                ...
                                <div className="flex justify-between items-start mb-10">
                                    <div className="flex items-center gap-4">
                                        <div className="w-14 h-14 rounded-2xl bg-[#0a0a0a] border border-[#ffffff0a] flex items-center justify-center text-3xl shadow-inner">{maker?.avatar || '👤'}</div>
                                        <div>
                                            <p className="text-sm font-black text-white tracking-tight">{sanitizeName(maker?.username)}</p>
                                            <div className="flex items-center gap-2 mt-1.5">
                                                <span className="text-[8px] font-black text-[#10b981] bg-[#10b981]/10 px-2 py-0.5 rounded-full uppercase tracking-tighter">{stats.winRate}% Accuracy</span>
            ...
            <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-500">
                <div className="flex justify-between items-end mb-10 px-6">
                    <div>
                        ...
                        <p className="text-[#C5A880] text-[10px] font-black uppercase tracking-[0.3em] mt-2">Elite Traders • Finalizing Mar 18</p>
                    </div>
                    <Trophy className="w-10 h-10 text-yellow-600 opacity-50" />
                </div>
                {sortedLeaderboard.map((user, i) => (
                    <div key={user.id} className={`bg-[#111111] border ${user.id === session?.user?.id ? 'border-[#C5A880]/40 bg-[#C5A880]/5' : 'border-[#ffffff05]'} rounded-[2rem] p-6 flex items-center justify-between group transition-all`}>
                        ...
                        <div className="text-right">
                            <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest mb-1">Net Worth</p>
        ...
        {activeView === 'wallet' && (
            <div className="max-w-2xl mx-auto animate-in fade-in">
                <div className="bg-gradient-to-br from-[#1a1a1a] to-[#0a0a0a] border border-[#ffffff10] rounded-[2.5rem] p-10 mb-10 relative overflow-hidden shadow-2xl">
                    <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.3em] mb-4">Current Bankroll</p>
                    ...
                    <div className="flex gap-4">
                        <button className="flex-1 bg-[#C5A880] text-black font-black py-4 rounded-2xl uppercase text-[10px] tracking-widest flex items-center justify-center gap-2"><ArrowDownToLine className="w-4 h-4" /> Deposit</button>
                ...
                <div className="flex items-center gap-2 mb-6 text-gray-500"><History className="w-4 h-4" /><h3 className="text-xs font-black uppercase tracking-widest">Transaction Ledger</h3></div>
                <div className="bg-[#111111] border border-[#ffffff05] rounded-[2rem] p-10 text-center text-gray-700 text-[10px] font-black uppercase tracking-widest">
                    <CheckCircle2 className="w-8 h-8 mx-auto mb-4 opacity-10" /> Ledger Is Synchronized
                </div>
            </div>
        )}
        ...
        {activeView === 'wagers' && (
            <div className="max-w-4xl mx-auto animate-in fade-in">
                <h2 className="text-3xl font-black text-white uppercase tracking-tighter mb-10 italic">Your Arena</h2>
                <div className="bg-[#111111] border border-[#ffffff0a] rounded-[3rem] p-16 text-center border-dashed">
                    ...
                    <p className="text-gray-600 font-black uppercase tracking-[0.3em] text-xs mb-8">No Active Match Found</p>
                    <button onClick={() => setActiveView('markets')} className="bg-white/5 border border-white/10 px-10 py-4 rounded-2xl text-[10px] font-black uppercase hover:bg-[#C5A880] hover:text-black transition-all">Match Someone</button>
                </div>
                <div className="mt-20 flex items-center gap-4 border-t border-[#ffffff05] pt-10 opacity-20 grayscale">
                    <Bell className="w-6 h-6" /><p className="text-xs font-black uppercase tracking-widest">Real-time match alerts enabled</p>
                </div>
            </div>
        )}
      </main>
      ...
      {offerToMatch && (
        <div className="fixed inset-0 z-[250] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
            <div className="bg-[#111111] border border-[#C5A880]/30 rounded-[3rem] p-10 max-w-md w-full text-center relative shadow-2xl overflow-hidden">
                ...
                <h2 className="text-3xl font-black mb-4 uppercase tracking-tighter italic">Execute?</h2>
                <p className="text-gray-400 text-sm mb-12 leading-relaxed px-4">Matching this wager commits <span className="text-white font-black">{Math.round((offerToMatch.stake * (offerToMatch.odds || 2)) - offerToMatch.stake).toLocaleString()} KSh</span> into protocol escrow.</p>
                <div className="flex gap-4">
                    <button onClick={() => setOfferToMatch(null)} className="flex-1 py-5 rounded-2xl bg-[#1a1a1a] text-gray-500 font-black uppercase tracking-[0.2em] text-[10px]">Decline</button>
                    <button onClick={confirmMatch} className="flex-1 py-5 rounded-2xl bg-[#10b981] text-black font-black uppercase tracking-[0.2em] text-[10px] shadow-lg hover:scale-105 transition-transform">Accept</button>
                </div>
            </div>
        </div>
      )}
                ...
                <div className="space-y-10">
                    <div className="grid grid-cols-2 gap-4">
                        {events.find(e => e.id === p2pSelectedEventId)?.outcomes.map((o, idx) => (
                            <button key={idx} onClick={() => setP2pSelectedOutcomeIdx(idx)} className={`py-5 rounded-[1.5rem] border-2 font-black text-xs uppercase transition-all duration-500 ${p2pSelectedOutcomeIdx === idx ? 'bg-[#C5A880] text-black border-[#C5A880]' : 'bg-transparent border-[#ffffff10] text-gray-700'}`}>{o}</button>
                        ))}
                    </div>
                    <div className="grid grid-cols-2 gap-8">
                        <div>
                            <label className="text-[9px] font-black text-gray-600 uppercase mb-4 block tracking-[0.2em]">Stake (KSh)</label>
                            <input type="number" value={p2pStake} onChange={e => setP2pStake(Number(e.target.value))} className="w-full bg-[#0a0a0a] border border-[#ffffff0a] rounded-2xl p-5 font-mono font-black text-xl focus:border-[#C5A880] outline-none text-white shadow-inner" />
                        </div>
                        <div>
                            <label className="text-[9px] font-black text-gray-600 uppercase mb-4 block tracking-[0.2em]">Odds Target</label>
                            <input type="number" step="0.1" value={p2pOdds} onChange={e => setP2pOdds(Number(e.target.value))} className="w-full bg-[#0a0a0a] border border-[#ffffff0a] rounded-2xl p-5 font-mono font-black text-xl focus:border-[#C5A880] outline-none text-[#C5A880] shadow-inner" />
                        </div>
                    </div>
                    ...
                    }} className="w-full bg-white text-black font-black py-7 rounded-[2rem] uppercase tracking-[0.4em] text-[11px] shadow-2xl hover:bg-[#C5A880] transition-all border-b-8 border-gray-300">Push to Board</button>
                </div>
            </div>
        </div>
      )}
