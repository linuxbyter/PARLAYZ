...
export default function App() {
  ...
  const submitPoolBet = async (overrideE?: string, overrideO?: number, overrideS?: number) => {
    ...
    } else showToast('Network error pushing wager.') 
  }
  ...
  const calculateEstPayout = (eventId: string, outcomeIdx: number, userStake: number = 0, isExisting: boolean = false) => {
    ...
    return Math.round(((userStake / winningPoolVolume) * totalPoolVolume) * (1 - PLATFORM_FEE_PERCENT / 100))
  }
  ...
  const sanitizeName = (name: string | undefined) => !name ? 'Anonymous' : name.includes('@') ? name.split('@')[0] : name
  ...
  const activeEvents = events.filter(e => {
    ...
    if (selectedMainCategory !== 'All') {
      ...
      else matchesCategory = e.category.startsWith(`${selectedMainCategory.replace(/\s+/g, '')}_`);
    }
  ...
  if (loading) return <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center relative"><div className="w-16 h-16 border-2 border-[#C5A880]/20 border-t-[#C5A880] rounded-full animate-spin mb-6"></div><p className="text-[#C5A880] font-mono text-xs tracking-[0.3em] uppercase animate-pulse">Syncing Network...</p></div>
      ...
      {showProfileSetup && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-[#0a0a0a]/95 backdrop-blur-xl p-4">
          <div className="bg-[#111111] border border-[#C5A880]/30 rounded-3xl p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-300 relative overflow-hidden">
            ...
            <p className="text-gray-400 text-sm mb-8 font-light relative z-10">You are entering a social exchange.</p>
            ...
            {(() => {
              ...
              return (
                <div className="max-w-5xl mx-auto">
                  <button onClick={() => setActiveView('markets')} className="flex items-center gap-2 text-gray-500 hover:text-[#C5A880] transition mb-6 font-bold text-xs uppercase tracking-widest bg-[#111111] border border-[#ffffff10] hover:border-[#C5A880]/30 px-4 py-2 rounded-xl"><ChevronLeft className="w-4 h-4" /> Return</button>
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
                    <div className="lg:col-span-2 space-y-6 sm:space-y-8">
                      <div>
                        <div className="flex flex-wrap items-center gap-3 mb-4">
                          <span className="text-xs font-bold text-[#C5A880] uppercase tracking-wider bg-[#C5A880]/10 border border-[#C5A880]/20 px-3 py-1.5 rounded-lg shadow-sm">{event.category.replace('_', ' • ')}</span>
                          <LiveTimer targetDate={eventDateStr} />
                          <button onClick={() => setChatEventId(event.id)} className="flex items-center gap-2 text-xs font-bold text-[#f43f5e] uppercase tracking-wider bg-[#f43f5e]/10 border border-[#f43f5e]/20 px-3 py-1.5 rounded-lg shadow-sm hover:bg-[#f43f5e]/20 transition"><MessageSquare className="w-3.5 h-3.5" /> Warzone Chat</button>
                        </div>
                      ...
                      <div className="bg-[#111111] border border-[#ffffff10] rounded-3xl p-6 sm:p-8 relative overflow-hidden">
                        ...
                        {event.settlement_source && <div className="mt-4 pt-4 border-t border-[#ffffff10] text-[10px] text-gray-500 uppercase tracking-widest flex items-center gap-1.5 relative z-10"><CheckCircle2 className="w-3.5 h-3.5 text-[#C5A880]" />Result verified via: <span className="text-white font-semibold">{event.settlement_source}</span></div>}
                      </div>
                              ...
                              <div className="grid gap-2">
                                {event.outcomes.map((outcome, idx) => (
                                  <button key={idx} onClick={() => setSelectedOutcomeIdx(idx)} className={`p-4 rounded-xl border text-sm font-bold transition text-left ${selectedOutcomeIdx === idx ? 'bg-[#C5A880]/10 border-[#C5A880] text-[#C5A880]' : 'bg-[#0a0a0a] border-[#ffffff10] text-gray-300 hover:bg-[#1a1a1a]'}`}>
                                    <div className="flex justify-between items-center"><span>{outcome}</span>{selectedOutcomeIdx === idx && <CheckCircle2 className="w-4 h-4" />}</div>
                                  </button>
                                ))}
                              </div>
                            </div>
                            ...
                            {betMode === 'p2p' ? (
                              <div className="space-y-4 animate-in fade-in duration-300">
                                <div className="flex items-center justify-between mb-1">
                                  <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest flex items-center gap-1">Custom Odds <button onClick={() => alert("Custom Odds let YOU set the price. If you risk more than you ask from a friend, you are giving them better odds. If no friend joins, the House matches you 1:1.")}><HelpCircle className="w-3 h-3 text-[#C5A880]" /></button></label>
                                </div>
                                ...
                                <button onClick={submitDuel} disabled={selectedOutcomeIdx === null || poolStake < MIN_STAKE || friendStake < MIN_STAKE} className="w-full bg-[#f43f5e] text-white font-black py-5 rounded-xl uppercase tracking-widest text-sm hover:scale-[1.02] transition">Create 1v1 Duel</button>
                              </div>
                            ) : (
                              <>
                                <div>
                                  <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">2. Stake Amount (KSh)</label>
                                  <input type="number" min={MIN_STAKE} value={poolStake || ''} onChange={(e) => setPoolStake(Number(e.target.value))} className="w-full bg-[#0a0a0a] border border-[#ffffff15] text-white font-black rounded-xl p-4 focus:outline-none focus:border-[#C5A880] text-2xl text-center" placeholder="0" />
                                </div>
                                <div className="bg-[#0a0a0a] rounded-xl p-5 border border-[#ffffff0a]">
                                  <span className="text-[#10b981] font-bold uppercase tracking-wider text-xs">Est. Gross Payout:</span><br/>
                                ...
                                <button onClick={() => submitPoolBet()} disabled={selectedOutcomeIdx === null || poolStake < MIN_STAKE} className="w-full bg-[#C5A880] text-[#0a0a0a] font-black py-5 rounded-xl transition uppercase tracking-widest text-sm hover:scale-[1.02]">Confirm Liquidity</button>
                              </>
                            )}
                          </div>
                        )}
          ...
          <div className="max-w-2xl mx-auto animate-in fade-in duration-300">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowProfileSetup(true)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border bg-[#1a1a1a] border-[#ffffff20] text-gray-300 hover:text-white text-xs font-semibold uppercase tracking-wider transition">Edit Identity</button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-10">
              <div className="bg-[#111111] border border-[#ffffff10] rounded-2xl p-6 relative overflow-hidden group hover:border-[#C5A880]/30 transition">
                <div className="flex items-center gap-3 mb-6 relative z-10"><div className="w-10 h-10 rounded-lg bg-[#C5A880]/10 flex items-center justify-center border border-[#C5A880]/20"><ArrowDownToLine className="w-5 h-5 text-[#C5A880]" /></div><h3 className="text-lg font-bold">Deposit</h3></div>
                <div className="space-y-4 relative z-10">
                  <input type="tel" placeholder="07XXXXXXXX" value={depositPhone} onChange={e => setDepositPhone(e.target.value.replace(/\D/g, '').substring(0,10))} className="w-full bg-[#0a0a0a] border border-[#ffffff15] text-white rounded-xl p-3 focus:outline-none focus:border-[#C5A880] font-mono tracking-wider" />
                  <input type="number" placeholder="Amount" value={depositAmount || ''} onChange={e => setDepositAmount(Number(e.target.value))} className="w-full bg-[#0a0a0a] border border-[#ffffff15] rounded-xl p-3 focus:outline-none focus:border-[#C5A880] font-bold text-white" />
                  <button onClick={handleDeposit} disabled={isDepositing} className="w-full bg-[#C5A880]/10 border border-[#C5A880]/30 hover:bg-[#C5A880] text-[#C5A880] hover:text-[#0a0a0a] font-bold py-3.5 rounded-xl transition">{isDepositing ? '...' : 'STK Push'}</button>
                </div>
              </div>
              ...
              <div className="bg-[#111111] border border-[#ffffff10] rounded-2xl p-6 relative overflow-hidden group hover:border-[#10b981]/30 transition">
                ...
                <div className="space-y-4 relative z-10">
                  <input type="tel" placeholder="07XXXXXXXX" value={withdrawPhone} onChange={e => setWithdrawPhone(e.target.value.replace(/\D/g, '').substring(0,10))} className="w-full bg-[#0a0a0a] border border-[#ffffff15] text-white rounded-xl p-3 focus:outline-none focus:border-[#10b981] font-mono tracking-wider" />
                  <input type="number" placeholder="Amount" value={withdrawAmount || ''} onChange={e => setWithdrawAmount(Number(e.target.value))} className="w-full bg-[#0a0a0a] border border-[#ffffff15] rounded-xl p-3 focus:outline-none focus:border-[#10b981] font-bold text-white" />
              ...
              <div className={`bg-[#111111] border border-[#ffffff10] rounded-2xl p-6 relative overflow-hidden transition md:col-span-2 lg:col-span-1 ${profile?.has_claimed_airdrop ? 'opacity-60 grayscale' : 'hover:border-[#C5A880]/30'}`}>
                ...
                <div className="space-y-4 relative z-10">
                  ...
                  <button onClick={handleAirdrop} disabled={profile?.has_claimed_airdrop} className={`w-full font-bold py-3.5 rounded-xl transition ${profile?.has_claimed_airdrop ? 'bg-[#0a0a0a] border border-[#ffffff10] text-gray-600' : 'bg-[#C5A880]/10 border border-[#C5A880]/30 text-[#C5A880] hover:bg-[#C5A880] hover:text-[#0a0a0a]'}`}>
            ...
            <div>
              ...
              <div className="bg-[#111111] border border-[#ffffff10] rounded-2xl overflow-hidden">
                {ledgerTransactions.length === 0 ? <div className="p-8 text-center text-gray-500 font-light">No records found.</div> : (
                    ...
                    {ledgerTransactions.map((tx) => {
                      ...
                      return (
                        <div key={tx.id} className="p-4 sm:p-5 flex items-center justify-between hover:bg-[#1a1a1a] transition">
                          ...
                          <div className="text-right"><p className={`font-bold ${isPositive ? 'text-[#10b981]' : 'text-[#f43f5e]'}`}>{isPositive ? '+' : '-'}{tx.message.match(/\d+/) ? tx.message.match(/\d+/)?.[0] : ''} KSh</p></div>
                        </div>
                      )
                    })}
                  </div>
          ...
          <div className="max-w-3xl mx-auto animate-in fade-in duration-300">
            <div className="bg-gradient-to-r from-yellow-500/10 via-yellow-500/5 to-transparent border border-yellow-500/30 rounded-2xl p-4 sm:p-6 mb-8 text-center flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-left">
                ...
                <p className="text-yellow-500/80 text-sm font-semibold">Ends March 31st, 2026. Top traders win 1500 KSh every sunday.</p>
              </div>
            </div>
            ...
            <div className="bg-[#111111] border border-[#ffffff10] rounded-3xl overflow-hidden shadow-2xl">
              <div className="grid grid-cols-12 gap-4 p-5 border-b border-[#ffffff10] bg-[#0a0a0a] text-xs font-bold text-gray-500 uppercase tracking-widest">
                <div className="col-span-2 text-center">Rank</div><div className="col-span-6">Trader Identity</div><div className="col-span-4 text-right">Net Value</div>
              </div>
              <div className="divide-y divide-[#ffffff0a]">
                {sortedLeaderboard.map((user, index) => (
                  <div key={user.id} onClick={() => setSelectedPublicProfile(user)} className={`grid grid-cols-12 gap-4 p-5 items-center transition cursor-pointer hover:bg-[#1a1a1a] ${session?.user?.id === user.id ? 'bg-[#C5A880]/5' : ''}`}>
                    <div className="col-span-2 flex justify-center">
                      {index === 0 ? <span className="w-8 h-8 rounded-full bg-yellow-500/20 text-yellow-500 border border-yellow-500/50 flex items-center justify-center font-black">1</span> :
                       index === 1 ? <span className="w-8 h-8 rounded-full bg-gray-300/20 text-gray-300 flex items-center justify-center font-black">2</span> :
                       index === 2 ? <span className="w-8 h-8 rounded-full bg-orange-600/20 text-orange-500 flex items-center justify-center font-black">3</span> :
                    ...
                    <div className="col-span-6 flex items-center gap-3">
                      ...
                      <div>
                        <span className={`font-bold block ${session?.user?.id === user.id ? 'text-[#C5A880]' : 'text-white'}`}>{sanitizeName(user.username)} {session?.user?.id === user.id && '(You)'}</span>
        ...
        {activeView === 'wagers' && (
          <div className="space-y-10 animate-in fade-in duration-300">
            ...
            <div>
              ...
              {myActiveWagers.length === 0 ? <div className="py-16 text-center text-gray-500 border border-dashed border-[#ffffff10] rounded-2xl">No active predictions.</div> : (
                    ...
                    return (
                      <div key={i} className="bg-[#111111] border border-[#C5A880]/40 rounded-3xl p-6 transition relative overflow-hidden shadow-[0_0_30px_rgba(197,168,128,0.05)]">
                        <div className="flex items-center justify-between mb-4 pb-4 border-b border-[#ffffff10] relative z-10">
                          <div><p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Status</p><p className="font-bold text-[#10b981] text-sm flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#10b981] animate-pulse"></span> Live</p></div>
                        </div>
                        <h3 className="text-lg font-bold text-white mb-4 relative z-10 leading-tight">{event.title}</h3>
                        <div className="bg-[#0a0a0a] rounded-xl p-4 border border-[#ffffff0a] mb-4 relative z-10 text-center"><div className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-1">Prediction</div><div className="text-[#C5A880] font-bold text-lg">{event.outcomes[bet.outcome_index]}</div></div>
                        <div className="space-y-2 text-sm relative z-10">
                          <div className="flex justify-between text-gray-400"><span>Stake:</span><span className="text-white">{bet.stake.toLocaleString()} KSh</span></div>
                          <div className="flex justify-between font-bold pt-3 border-t border-[#ffffff10] mt-3"><span className="text-[#10b981] uppercase tracking-wider text-xs">Est. Payout:</span><span className="text-[#10b981] text-xl">{calculateEstPayout(event.id, bet.outcome_index, bet.stake, true).toLocaleString()} KSh</span></div>
                        </div>
                      </div>
                    )
                  })}
              ...
              {mySettledWagers.length === 0 ? <div className="py-10 text-center text-gray-500 border border-dashed border-[#ffffff10] rounded-2xl">No settled history yet.</div> : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {mySettledWagers.reverse().map((bet, i) => {
                    ...
                    return (
                      <div key={i} className={`bg-[#111111] border rounded-3xl p-6 relative overflow-hidden transition ${isWin ? 'border-[#10b981]/50 shadow-[0_0_30px_rgba(16,185,129,0.1)]' : isRefund ? 'border-gray-600' : 'border-[#f43f5e]/20 opacity-70'}`}>
          ...
          <div className="animate-in fade-in duration-300">
            <div className="mb-6 relative max-w-md">
              ...
              <input 
                ...
                className="w-full bg-[#111111] border border-[#ffffff15] text-white rounded-2xl pl-11 pr-4 py-3 focus:outline-none focus:border-[#C5A880] text-sm"
              />
            </div>

            <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar mb-2 select-none">
              {Object.keys(CATEGORIES).map((cat) => (
                <button key={cat} onClick={() => { setSelectedMainCategory(cat); setSelectedSubCategory(null); }} className={`px-5 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition border whitespace-nowrap flex items-center gap-2 ${selectedMainCategory === cat ? 'bg-[#C5A880] text-[#0a0a0a] border-[#C5A880] shadow-[0_0_15px_rgba(197,168,128,0.3)]' : 'bg-[#111111] text-gray-400 border-[#ffffff10] hover:border-[#C5A880]/50 hover:text-white'}`}>
                  {cat === 'Current Affairs' && <Globe className="w-3.5 h-3.5" />} {cat}
                </button>
              ))}
            </div>

            {selectedMainCategory !== 'All' && CATEGORIES[selectedMainCategory as keyof typeof CATEGORIES].length > 0 && (
              <div className="flex gap-2 overflow-x-auto pb-6 no-scrollbar mb-2 animate-in slide-in-from-top-2 duration-200 select-none">
                <button onClick={() => setSelectedSubCategory(null)} className={`px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase transition border whitespace-nowrap ${selectedSubCategory === null ? 'bg-[#333] text-white border-gray-500' : 'bg-transparent text-gray-500 border-[#ffffff10] hover:text-white'}`}>
                  All {selectedMainCategory}
                </button>
                {CATEGORIES[selectedMainCategory as keyof typeof CATEGORIES].map((sub) => {
                  const isWar = sub.dbValue === 'CurrentAffairs_War';
                  const isSelected = selectedSubCategory === sub.dbValue;
                  return (
                    <button key={sub.dbValue} onClick={() => setSelectedSubCategory(sub.dbValue)} className={`px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase transition border whitespace-nowrap flex items-center gap-1 ${isSelected ? (isWar ? 'bg-[#f43f5e] text-white border-[#f43f5e]' : 'bg-[#1a1a1a] text-white border-gray-400') : (isWar ? 'text-[#f43f5e]/80 border-[#f43f5e]/30 hover:border-[#f43f5e]' : 'bg-transparent text-gray-500 border-[#ffffff10] hover:text-white')}`}>
                      {isWar && <AlertTriangle className="w-3 h-3" />} {sub.label}
                    </button>
                  )
                })}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {activeEvents.length === 0 ? <div className="col-span-full py-16 text-center text-gray-500 border border-dashed border-[#ffffff10] rounded-3xl bg-[#111111]/30">No active markets found.</div> : (
                activeEvents.map((event) => {
                  const eventBets = bets.filter(b => b.event_id === event.id && b.status === 'open')
                  const totalPoolVolume = eventBets.reduce((sum, b) => sum + b.stake, 0)
                  const lockTime = new Date(event.closes_at || event.locks_at || '').getTime();
                  const isLocked = !isNaN(lockTime) && lockTime <= Date.now();

                  return (
                    <div key={event.id} onClick={() => { setSelectedEventId(event.id); setSelectedOutcomeIdx(null); setActiveView('eventDetail'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="bg-[#111111] border border-[#ffffff10] rounded-3xl p-5 transition flex flex-col group relative overflow-hidden select-none cursor-pointer hover:border-[#C5A880]/50 hover:-translate-y-1 shadow-lg">
                      <div className="flex items-start justify-between mb-3 relative z-10 pointer-events-none">
                        <span className="text-[10px] font-bold text-[#C5A880] uppercase tracking-wider bg-[#C5A880]/10 border border-[#C5A880]/20 px-2 py-1 rounded-lg shadow-sm">
                          {event.category.split('_').join(' • ').replace('PlayerBets', 'Player Bets').replace('TVFilm', 'TV & Film').replace('CurrentAffairs', 'Current Affairs')}
                        </span>
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4 relative z-10 flex-grow pointer-events-none">
                        {event.outcomes.map((outcome, idx) => {
                          ...
                          return (
                            <div key={idx} className={`flex ${isOddLast ? 'flex-row gap-4 items-center justify-between col-span-2 py-2 px-4' : 'flex-col items-center justify-center p-3'} rounded-xl bg-[#0a0a0a]/80 backdrop-blur-md transition-all duration-700 relative overflow-hidden`} style={{ borderColor: `rgba(${rgb}, ${totalPoolVolume === 0 ? 0.1 : 0.2 + (percent / 100) * 0.8})`, borderWidth: '1px' }}>
                              <span className={`text-xs font-bold text-white text-center drop-shadow-md ${isOddLast ? '' : 'mb-1'} line-clamp-1`}>{outcome}</span>
                              <div className={`flex ${isOddLast ? 'items-center gap-3' : 'flex-col items-center'}`}><span className={`${isOddLast ? 'text-base' : 'text-lg'} font-black tracking-tight`} style={{ color: `rgba(${rgb}, 1)` }}>{percent}%</span><span className="text-[9px] text-gray-500 uppercase tracking-widest">{outcomeVolume.toLocaleString()} KSh</span></div>
                            </div>
                          )
                        })}
                      </div>
                      <div className="flex justify-between items-center text-[11px] font-semibold text-gray-500 uppercase tracking-widest mb-3 pt-3 border-t border-[#ffffff0a] relative z-10 pointer-events-none"><span>Liquidity:</span><span className="text-white font-mono">{totalPoolVolume.toLocaleString()} KSh</span></div>
                      <div className="flex gap-2 mt-auto relative z-20">
                        <div className={`flex-grow font-bold py-2.5 rounded-xl transition flex items-center justify-center gap-2 shadow-sm uppercase tracking-widest text-xs pointer-events-none ${isLocked ? 'bg-[#0a0a0a] border border-red-500/20 text-red-500' : 'bg-[#1a1a1a] border border-[#ffffff15] text-white group-hover:bg-[#C5A880] group-hover:text-[#0a0a0a]'}`}>{isLocked ? '🔒 Locked' : 'Trade Pool ⚔️'}</div>
                        <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setChatEventId(event.id); }} className="w-12 bg-[#1a1a1a] hover:bg-[#f43f5e] hover:text-white border border-[#ffffff15] hover:border-[#f43f5e] text-gray-400 rounded-xl transition flex items-center justify-center shadow-sm" title="Warzone Chat"><MessageSquare className="w-4 h-4" /></button>
                      </div>
                    </div>
                  )
                })
            ...
            <div className="relative z-10">
              {showCashierModal.status === 'processing' ? (
                ...
                <>
                  ...
                  <button onClick={() => setShowCashierModal(null)} className="w-full bg-[#1a1a1a] hover:bg-[#222222] border border-[#ffffff10] text-white font-bold py-3.5 rounded-xl transition mt-4">Return</button>
                </>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="bg-[#111111] border border-[#10b981]/30 rounded-3xl p-6 sm:p-8 w-full max-w-sm text-center shadow-[0_0_50px_rgba(16,185,129,0.1)] relative overflow-hidden">
            ...
            <div className="space-y-2 mt-6">
              <button onClick={copyChallengeLink} className="w-full bg-[#C5A880] text-black font-black py-4 rounded-xl flex items-center justify-center gap-2 hover:scale-105 transition-all uppercase text-xs tracking-widest shadow-xl"><Share2 className="w-4 h-4"/> Share Challenge Link</button>
