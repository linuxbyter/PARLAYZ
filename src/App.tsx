...
function App() {
  ...
  const placeBet = async (eventId: string, outcomeIndex: number) => {
    ...
    if (error) {
      ...
      alert(`Bet placed! ${betAmount} credits on outcome ${outcomeIndex + 1}`)
      fetchBets()
      setSelectedEvent(null)
    }
  }

  const getOddsForOutcome = (eventId: string, outcomeIndex: number) => {
    ...
    const allEventBets = bets.filter(b => b.event_id === eventId)
    const totalEventStake = allEventBets.reduce((sum, b) => sum + b.stake, 0)
    
    if (totalEventStake === 0) return 50
    return Math.round((totalStake / totalEventStake) * 100)
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {events.map((event) => (
            <EventCard
              ...
              event={event}
              bets={bets}
              getOdds={getOddsForOutcome}
              onBet={(outcomeIdx) => placeBet(event.id, outcomeIdx)}
            />
          ))}
        </div>
      </main>
...
function EventCard({ event, bets, getOdds, onBet }: {
  event: Event
  bets: Bet[]
  getOdds: (eventId: string, outcomeIdx: number) => number
  onBet: (outcomeIdx: number) => void
}) {
  ...
  return (
    <div className="bg-matte-800 border border-matte-700 rounded-2xl p-6 hover:border-gold-500/50 transition">
