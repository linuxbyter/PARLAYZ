...
export default function App() {
  ...
  const handleAISend = async () => {
    ...
    if (input.includes('how') || input.includes('work')) {
        response = "We are a Peer-to-Peer exchange. You don't bet against us; you bet against other people in Kenya. You set the odds, others match them."
    } else if (input.includes('fee')) {
        response = `We charge a small ${PLATFORM_FEE_PERCENT}% fee on winning payouts only. This helps us keep the platform running!`
    }
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-[#C5A880]/30 overflow-x-hidden">
      ...
      {toast && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[200] animate-in slide-in-from-top-10">
          <div className={`px-6 py-3 rounded-2xl border shadow-2xl backdrop-blur-xl flex items-center gap-3 ${toast.type === 'success' ? 'bg-[#10b981]/10 border-[#10b981]/30 text-[#10b981]' : 'bg-[#f43f5e]/10 border-[#f43f5e]/30 text-[#f43f5e]'}`}>
      ...
      <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end gap-4">
        {showAI && (
            <div className="w-80 h-[450px] bg-[#111111] border border-[#C5A880]/30 rounded-[2rem] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 zoom-in-95 duration-300">
                <div className="p-5 bg-[#1a1a1a] border-b border-[#ffffff10] flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#C5A880]">Support Agent</span>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar bg-[#0a0a0a]/30">
                    ...
                    {isTyping && <div className="text-[9px] text-gray-600 font-black uppercase tracking-widest ml-1 animate-pulse">Thinking...</div>}
                    <div ref={aiEndRef} />
                </div>
                <div className="p-4 bg-[#111111] border-t border-[#ffffff05] flex gap-2">
                    <input value={aiInput} onChange={e => setAiInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAISend()} placeholder="Message AI..." className="flex-1 bg-[#0a0a0a] border border-[#ffffff10] rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-[#C5A880] transition" />
        ...
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
            <div className="flex items-center gap-8">
                ...
                <div className="hidden lg:flex gap-2 items-center bg-[#111111] p-1 rounded-full border border-[#ffffff05]">
                    ...
                    {CATEGORIES.map(c => (
                        <button key={c} onClick={() => setActiveCategory(c)} className={`px-5 py-2 rounded-full text-[10px] font-black uppercase transition-all duration-300 ${activeCategory === c ? 'bg-[#C5A880] text-[#0a0a0a]' : 'text-gray-500 hover:text-gray-300'}`}>{c}</button>
                    ))}
                </div>
            </div>
            <div className="flex items-center gap-4">
                ...
                <div className="w-10 h-10 rounded-2xl bg-[#1a1a1a] flex items-center justify-center text-xl border border-[#ffffff10] relative group">
                    ...
                    <div className="absolute top-11 right-0 hidden group-hover:block bg-[#111111] border border-[#ffffff10] rounded-xl p-3 z-[60] shadow-2xl">
                        <button onClick={() => supabase.auth.signOut()} className="flex items-center gap-2 text-[10px] text-red-500 uppercase font-black whitespace-nowrap"><LogOut className="w-3 h-3" /> Logout</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-in fade-in">
                {bets.filter(b => b.status === 'p2p_open').length === 0 ? (
                    <div className="col-span-full py-32 text-center bg-[#111111]/50 rounded-[3rem] border border-dashed border-[#ffffff0a]">
                        ...
                        <p className="text-gray-600 font-black uppercase tracking-[0.3em] text-xs">No Match Liquidity</p>
                    </div>
                ) : (
                        ...
                        return (
                            <div key={offer.id} className="bg-[#111111] border border-[#ffffff0a] rounded-[2.5rem] p-8 relative overflow-hidden group hover:border-[#C5A880]/40 transition-all duration-500 shadow-2xl">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2 mt-1.5">
                                                <span className="text-[8px] font-black text-[#10b981] bg-[#10b981]/10 px-2 py-0.5 rounded-full uppercase tracking-tighter">{stats.winRate}% Win Rate</span>
                                ...
                                <div className="bg-[#0a0a0a] rounded-3xl p-6 mb-10 border border-[#ffffff05] text-center shadow-inner relative">
                                    <p className="text-[9px] text-gray-600 uppercase font-black mb-3 tracking-[0.2em]">Required Stake</p>
                                    <p className="text-white font-black text-xl">{event.outcomes[offer.outcome_index]}</p>
                                </div>
                                <div className="flex justify-between items-center mb-10 px-2">
                                    <div><p className="text-[9px] text-gray-600 font-black uppercase tracking-widest">To Pay</p><p className="text-xl font-black text-white">{liability.toLocaleString()} <span className="text-xs text-gray-700 font-bold uppercase">KSH</span></p></div>
                                    <div className="text-right"><p className="text-[9px] text-[#10b981] font-black uppercase tracking-widest">Payout</p><p className="text-xl font-black text-[#10b981]">{(offer.stake + liability).toLocaleString()} <span className="text-xs text-[#10b981]/50 font-bold uppercase">KSH</span></p></div>
                                </div>
                                <button onClick={() => initiateMatch(offer)} className="w-full bg-white text-black font-black py-5 rounded-3xl hover:bg-[#C5A880] transition-all uppercase tracking-[0.2em] text-[10px] shadow-xl active:scale-95 border-b-4 border-gray-300 hover:border-[#A3885C]">Take The Bet</button>
                            </div>
                        )
                    })
                )}
        ...
        {activeView === 'leaderboard' && (
            <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-500">
                ...
                {sortedLeaderboard.map((user, i) => (
                    <div key={user.id} className={`bg-[#111111] border ${user.id === session?.user?.id ? 'border-[#C5A880]/40 bg-[#C5A880]/5' : 'border-[#ffffff05]'} rounded-[2.5rem] p-6 flex items-center justify-between group transition-all`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-in fade-in">
                {filteredEvents.map(event => (
                    <div key={event.id} className="bg-[#111111] border border-[#ffffff0a] rounded-[2.5rem] p-8 hover:border-white/20 transition-all flex flex-col group relative overflow-hidden shadow-2xl">
                        ...
                        <button onClick={() => { setP2pSelectedEventId(event.id); setShowCreateOfferModal(true); }} className="w-full bg-[#1a1a1a] border border-[#ffffff10] text-white font-black py-5 rounded-[1.8rem] hover:bg-white hover:text-black transition-all uppercase text-[10px] tracking-[0.2em] relative z-10">Create Custody Offer</button>
                    </div>
                ))}
            </div>
        )}
      ...
      {offerToMatch && (
        <div className="fixed inset-0 z-[250] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
            <div className="bg-[#111111] border border-[#C5A880]/30 rounded-[3rem] p-10 max-w-md w-full text-center relative shadow-2xl overflow-hidden">
                ...
                <p className="text-gray-400 text-sm mb-12 leading-relaxed px-4 tracking-tight">You are risking <span className="text-white font-black">{Math.round((offerToMatch.stake * (offerToMatch.odds || 2)) - offerToMatch.stake).toLocaleString()} KSh</span> in this match. Funds are escrowed until resolution.</p>
                <div className="flex gap-4">
                    ...
                    <button onClick={confirmMatch} className="flex-1 py-5 rounded-2xl bg-[#10b981] text-black font-black uppercase tracking-[0.2em] text-[10px] shadow-lg hover:scale-105 active:scale-95 transition-all">Match Now</button>
                </div>
            </div>
        </div>
      )}
            ...
            <div className="bg-[#111111] border border-[#C5A880]/30 rounded-[3rem] p-10 max-w-md w-full relative shadow-2xl">
                ...
                <div className="space-y-10">
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {events.find(e => e.id === p2pSelectedEventId)?.outcomes.map((o, idx) => (
                                <button key={idx} onClick={() => setP2pSelectedOutcomeIdx(idx)} className={`py-5 rounded-[1.8rem] border-2 font-black text-xs uppercase transition-all duration-500 ${p2pSelectedOutcomeIdx === idx ? 'bg-[#C5A880] text-black border-[#C5A880] shadow-xl scale-105' : 'bg-transparent border-[#ffffff05] text-gray-700 hover:border-gray-600'}`}>{o}</button>
                            ))}
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-8">
                        ...
                        <div>
                            <label className="text-[9px] font-black text-gray-600 uppercase mb-4 block tracking-[0.2em]">Odds (Multiplier)</label>
                    ...
                    }} className="w-full bg-white text-black font-black py-7 rounded-[2.2rem] uppercase tracking-[0.4em] text-[11px] shadow-2xl hover:bg-[#C5A880] transition-all border-b-8 border-gray-300 active:translate-y-1 active:border-b-0">Post Offer</button>
                </div>
            </div>
        </div>
      )}
