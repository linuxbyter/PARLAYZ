...
import { TrendingUp, TrendingDown, Clock, Trophy, BarChart3, Flame, Calendar } from 'lucide-react'
    ...
    <div className={`bg-[#111] border border-[#2D2D2D] rounded-2xl overflow-hidden ${className}`}>
      ...
      <div className="p-4 pt-0 space-y-4">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-[#1a1a1a] rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-8 h-8 rounded-xl bg-gradient-to-r from-[#1E3A8A] to-[#3B82F6] flex items-center justify-center">
                    <span className="text-xs font-bold text-white">KSh</span>
                  </div>
              ...
              <div className="bg-[#1a1a1a] rounded-xl p-4">
                ...
                <p className="text-2xl font-black font-mono text-white">{stats.winRate}%</p>
                <div className="flex items-center gap-1 mt-1">
              ...
              <div className="bg-[#1a1a1a] rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-8 h-8 rounded-xl bg-gradient-to-r from-[#8B5CF6] to-[#7C3AED] flex items-center justify-center">
                    <BarChart3 className="w-4 h-4 text-white" />
                  </div>
                  <span className="text-[10px] text-gray-500 uppercase font-bold">Staked</span>
                </div>
                <p className="text-2xl font-black font-mono text-white">KSh {stats.totalStaked.toLocaleString()}</p>
                <span className="text-[10px] text-gray-500">{stats.activeBets} active bets</span>
              </div>

              <div className="bg-[#1a1a1a] rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${stats.totalPnL >= 0 ? 'bg-green-500/20' : 'bg-red-500/20'}`}>
                    {stats.totalPnL >= 0 ? (
                ...
                <p className={`text-2xl font-black font-mono ${stats.totalPnL >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {stats.totalPnL >= 0 ? '+' : ''}KSh {stats.totalPnL.toLocaleString()}
                </p>
            ...
            {activeBets.length > 0 && (
              <div className="bg-[#1a1a1a] rounded-xl p-4">
                ...
                <div className="space-y-2">
                  {activeBets.map(bet => (
                    <div key={bet.id} className="flex items-center justify-between bg-[#111] border border-[#2D2D2D] rounded-lg p-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-[#1E3A8A]/20 flex items-center justify-center">
                          <TrendingUp className="w-4 h-4 text-[#3B82F6]" />
                        </div>
                        <div>
                          <p className="text-xs font-bold text-white">{bet.selection}</p>
                          <p className="text-[10px] text-gray-500">{bet.sport} · {format(new Date(bet.timestamp), 'h:mm a')}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-xs font-bold font-mono text-white">KSh {bet.amount.toLocaleString()}</p>
        ...
        {activeTab === 'history' && (
          <div className="space-y-2">
            {recentBets.map(bet => (
              <div key={bet.id} className="flex items-center justify-between bg-[#1a1a1a] border border-[#2D2D2D] rounded-xl p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-white">{bet.selection}</span>
                    ...
                    <p className="text-[10px] text-gray-500 flex items-center gap-1 mt-0.5">
                      ...
                      {bet.sport} · {format(new Date(bet.timestamp), 'MMM d, h:mm a')}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold font-mono text-white">KSh {bet.amount.toLocaleString()}</p>
                  {bet.payout && (
                    <p className="text-[10px] text-green-400 font-mono">+KSh {bet.payout.toLocaleString()}</p>
                  )}
                  <p className="text-[10px] text-gray-500">@ {bet.odds}x</p>
                </div>
              </div>
          ...
          <div className="space-y-3">
            {[
              ...
              >
                ...
                <div className="text-right">
                  <p className={`text-sm font-bold font-mono ${entry.isYou ? 'text-[#1E3A8A]' : 'text-green-400'}`}>
                    {entry.pnl >= 0 ? '+' : ''}KSh {Math.abs(entry.pnl).toLocaleString()}
                  </p>
                  <div className="flex items-center gap-1 justify-end">
                    <span className="text-xs text-gray-500">Rank #{entry.rank}</span>
                  </div>
                </div>
              </div>
            ))}
